...
public class LunaticRave2ScoreDatabaseManager {
	...
	public LunaticRave2ScoreDatabaseManager(String path, String player, String rival)
			...
			if (!rs.next()) {
				...
				if (qr.query(conn, sql, new MapListHandler(), "score").size() == 0) {
					sql = "CREATE TABLE [score] ([hash] TEXT NOT NULL,"
							...
							+ "[great] INTEGER," + "[good] INTEGER,"
							+ "[bad] INTEGER," + "[poor] INTEGER,"
							+ "[totalnotes] INTEGER," + "[maxcombo] INTEGER,"
							+ "[minbp] INTEGER," + "[playcount] INTEGER,"
							+ "[clearcount] INTEGER," + "[failcount] INTEGER,"
							+ "[rank] INTEGER," + "[rate] INTEGER,"
							+ "[clear_db] INTEGER," + "[op_history] INTEGER,"
							+ "[scorehash] TEXT," + "[ghost] TEXT,"
							+ "[clear_sd] INTEGER," + "[clear_ex] INTEGER,"
							+ "[op_best] INTEGER," + "[random] INTEGER,"
							+ "[date] INTEGER," + "[rseed] INTEGER,"
	...
	public int updateRivalData(String rivalId, List<IRScoreData> scores) {
		...
		try {
			Connection con = DriverManager.getConnection("jdbc:sqlite:"
					+ rootpath + rivalpath + rivalId + ".db");
			con.setAutoCommit(false);
			Statement stmt = con.createStatement();

			for (IRScoreData score : scores) {
				// ハッシュが存在し、スコアの更新があったか？
				String sql = "SELECT hash FROM rival WHERE hash = '"
						+ score.getHash() + "' and (r_clear != "
						+ score.getClear() + " or r_maxcombo != "
						+ score.getCombo() + " or r_perfect != "
						+ score.getPg() + " or r_great != " + score.getGr()
						+ " or r_minbp != " + score.getMinbp() + ");";
				...
				if (isUpdate == true) {
					...
					sql = "UPDATE rival SET r_clear = " + score.getClear()
							...
							+ ", r_perfect = " + score.getPg() + ", r_great = "
							+ score.getGr() + ", r_good = " + score.getGd()
							+ ", r_bad = " + score.getBd() + ", r_poor = "
							+ score.getPr() + ", r_minbp = " + score.getMinbp()
							+ ", r_option = " + score.getOption()
							+ ", r_lastupdate = " + score.getLastupdate()
							+ " WHERE hash = '" + score.getHash() + "';";
					stmt.executeUpdate(sql);
					num++;
				} else {
					// ハッシュが存在するか？
					sql = "SELECT hash FROM rival WHERE hash = '"
							+ score.getHash() + "';";
					...
					if (isExist == false) {
						...
						sql = "INSERT INTO rival VALUES ('" + score.getHash()
								+ "'," + score.getClear() + ","
								+ score.getNotes() + "," + score.getCombo()
								+ "," + score.getPg() + "," + score.getGr()
								+ "," + score.getGd() + "," + score.getBd()
								+ "," + score.getPr() + "," + score.getMinbp()
								+ "," + score.getOption() + ","
								+ score.getLastupdate() + ");";
						stmt.executeUpdate(sql);
						num++;
	...
	public IRScoreData getScoreData(String playername, String hash,
			boolean grade) {
		Connection con = null;
		...
		try {
			...
			Logger.getGlobal().severe(
					"スコア" + playername + "取得時の例外:" + e.getMessage());
		} finally {
			if (con != null) {
				try {
					con.close();
	 ...
	 */
	public Map<String, IRScoreData> getScoreDatas(String playername,
			String[] hashes, boolean grade) {
	...
	public void setScoreData(String playername, IRScoreData score) {
		...
		try {
			...
			sql = "insert into score "
					+ "(hash, clear, perfect, great, good, bad, poor, totalnotes, maxcombo, "
					+ "minbp, playcount, clearcount, failcount, rank, rate, clear_db, "
					+ "op_history, scorehash, ghost, clear_sd, clear_ex, op_best, rseed, "
					+ "complete)"
					+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
			qr.update(con, sql, score.getHash(), score.getClear(),
					score.getPg(), score.getGr(), score.getGd(), score.getBd(),
					score.getPr(), score.getNotes(), score.getCombo(),
			...
			Logger.getGlobal().severe(
					"スコア" + playername + "更新時の例外:" + e.getMessage());
		} finally {
			if (con != null) {
				try {
					con.close();
	...
	public void setScoreData(String playername,
		...
		try {
			...
			for (String hash : map.keySet()) {
				...
				if (vs.length() > 0) {
					...
					qr.update(con, "UPDATE score SET " + vs + "WHERE hash = '"
							+ hash + "'");
				}
			}
			con.commit();
	 ...
	 * @return <ハッシュ, スコアデータ>のマップ
	 */
	public Map<String, IRScoreData> getRivalScoreDatas(
			String rivalname, String[] hashes) {
		Map<String, IRScoreData> result = new HashMap<String, IRScoreData>();
		Connection con = null;
		try {
			con = DriverManager.getConnection("jdbc:sqlite:" + rootpath
			...
			ResultSetHandler<List<IRScoreData>> rh = new BeanListHandler<IRScoreData>(
					IRScoreData.class);
			for (String hash : hashes) {
				List<IRScoreData> score = (List<IRScoreData>) qr
						.query(con,
								"SELECT r_clear as clear, CAST((r_perfect * 2 + r_great) * 9 / (r_totalnotes * 2) as int) as rank, "
										+ "r_perfect as pg,  r_great as gr, r_minbp as minbp FROM rival WHERE hash = '"
										+ hash + "'", rh);
				if (score.size() > 0) {
			...
			Logger.getGlobal().severe(
					"ライバルスコア" + rivalname + "取得時の例外:" + e.getMessage());
		} finally {
			if (con != null) {
				try {
					con.close();
	...
	public PlayerData getPlayerDatas(String playername) {
		...
		try {
			...
			Logger.getGlobal().severe(
					"プレイヤーデータ" + playername + "取得時の例外:" + e.getMessage());
		} finally {
			if (conn != null) {
				try {
					conn.close();
