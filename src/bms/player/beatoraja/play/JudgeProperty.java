...
public enum JudgeProperty {
    ...
    public long[] getJudge(NoteType notetype, int judgerank, int[] judgeWindowRate) {
    	return switch(notetype) {
			...
			case SCRATCH -> windowrule.create(scratch, judgerank, judgeWindowRate);
    ...
    public enum JudgeWindowRule {
        ...
        public long[] create(long[] org, int judgerank, int[] judgeWindowRate) {
    		...
    		for (int i = 0; i < Math.min(org.length, 4); i++) {
    			...
    			for(int j = 0;j < 2;j++) {
					if(fixmin != -1 && Math.abs(judge[i * 2 + j]) < Math.abs(judge[fixmin * 2 + j])) {
						judge[i * 2 + j] = judge[fixmin * 2 + j];
					}
					if(fixmax != -1 && Math.abs(judge[i * 2 + j]) > Math.abs(judge[fixmax * 2 + j])) {
						judge[i * 2 + j] = judge[fixmax * 2 + j];
					}
    			}
    		}
    		...
    		for (int i = 0; i < Math.min(org.length, 3); i++) {
    			for(int j = 0;j < 2;j++) {
					judge[i * 2 + j] = judge[i * 2 + j]*judgeWindowRate[i] / 100;
					if(Math.abs(judge[i * 2 + j]) > Math.abs(judge[6 + j])) {
						judge[i * 2 + j] = judge[6 + j];
					}
					if(i > 0 && Math.abs(judge[i * 2 + j]) < Math.abs(judge[i * 2 - 2 + j])) {
						judge[i * 2 + j] = judge[i * 2 - 2 + j];
					}
    			}
    		}
