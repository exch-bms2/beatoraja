...
public class ASIOProcessor implements AudioProcessor, AsioDriverListener {
	...
	public ASIOProcessor() {
		mixer = new AudioMixer(64);
	}
			...
			if (starttime == 0 && duration == 0) {
				...
				for (SliceWav slice : slicesound[id]) {
					if (slice.starttime == starttime && slice.duration == duration) {
						synchronized (this) {
							slice.playid = mixer.put(slice.wav);
						}
